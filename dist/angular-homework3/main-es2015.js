...
let HotelListComponent = class HotelListComponent {
    ...
    set selectedHotel(selectedHotel) {
    ...
    selectHotelType($event) {
        const hotelType = $event.target.getAttribute('data-id');
        if (!hotelType) {
            return;
        }
...
let HotelSocialInfoComponent = class HotelSocialInfoComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let HotelTypePipe = class HotelTypePipe {
    transform(hotels, hotelType) {
        if (!hotels) {
            return;
        }
        if (!hotelType) {
        ...
        return hotels.filter((hotel) => hotel.type === hotelType);
    }
};
...
let HotelWeatherComponent = class HotelWeatherComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
const hotels$ = Object(rxjs__WEBPACK_IMPORTED_MODULE_0__["of"])(hotels).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_1__["delay"])(1500));
